...
catch
{
    ...
    if ($EmailonError)
    {
        try
        {
            ...
            # Add More Email Attributes
            $MessageArguments.Subject = "SIS Student Sync - Error"
            $MessageArguments.Body = "There has been an error running the SIS Student Sync Script (Name: `"$($_.InvocationInfo.ScriptName)`" | Line: $($_.InvocationInfo.ScriptLineNumber)):`n`n$_`n`nThank you,`nThe IT Team"
            $MessageArguments.Attachment = $null # No attachments because we don't want anything to accidentally prevent the alert email from being sent.
