...
## [Unreleased](https://github.com/Sekers/SKYAPI/compare/0.5.0...develop)
...
- **BREAKING CHANGE:** The path set by `Set-SKYAPIConfigFilePath` is no longer available to scripts as the `$sky_api_config_file_path` variable. `Get-SKYAPIConfig`, `Set-SKYAPIConfig` and `Remove-SKYAPIConfig` still use that path when `-ConfigPath` is omitted, so a script that passed `-ConfigPath $sky_api_config_file_path` should leave `-ConfigPath` out instead.
