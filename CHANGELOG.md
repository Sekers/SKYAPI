...
- Piping a plain value, rather than an object with matching property names, into a function that has no required parameter now reports that the value matches no parameter instead of applying it to every text filter at once. This affects the roster and list functions such as `Get-SchoolActivityRoster` and `Get-SchoolCourse`, and also `Connect-SKYAPI`, `Disconnect-SKYAPI` and `Set-SKYAPIConfig`. Pass the value by name instead.
