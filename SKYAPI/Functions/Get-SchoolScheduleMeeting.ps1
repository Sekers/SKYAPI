function Get-SchoolScheduleMeeting
{
    <#
        ...
        Additional Notes:
          ...
          - Does not support the "show_time_for_current_date" request parameter. Setting it stamps the CURRENT day's UTC offset on every meeting, so a meeting in the other half of the year is reported an hour off (e.g., a January 08:30 EST class comes back as 08:30-04:00 when requested during EDT). If you need to convert time zones or DST, use PowerShell.
    ...
    process
    {
        ...
        $parameters = Get-SKYAPIRequestParameter -BoundParameters $PSBoundParameters -Exclude 'SchoolTimeZoneId','IncludeRosters' -SuppliedNames $SuppliedParameter
        ...
        # holding this record's date instead of getting its own 30 day window.
        [int]$IterationRangeInDays = 30
        $EffectiveEndDate = $end_date
        if ($null -eq $end_date -or $end_date -eq '' -or $end_date -eq 0)
        {
            $EffectiveEndDate = (([DateTime]$start_date).AddDays($IterationRangeInDays)).ToString('yyyy-MM-dd')
        }

        # Validate End Date String
        try {$null = [datetime]$EffectiveEndDate} catch
        {
            throw $_
        }

        # Validate that the start date is not after the end date.
        if ([datetime]$start_date -gt [datetime]$EffectiveEndDate)
        {
            throw "start_date ($start_date) cannot be after end_date ($EffectiveEndDate)."
        }

        # Initialize Variables
        $response = [System.Collections.Generic.List[Object]]::new()
        $DateRangeEnd = [DateTime]$EffectiveEndDate
        ...
        if ($IncludeRosters -and $response.Count -gt 0)
        {
            ...
            $OverlappingSchoolYears = Get-SchoolYear | Where-Object {
                (([datetime]$_.begin_date) -le ([datetime]$EffectiveEndDate)) -and (([datetime]$_.end_date) -ge ([datetime]$start_date))
            }
